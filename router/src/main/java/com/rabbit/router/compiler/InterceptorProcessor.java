 ...
 */
@SupportedAnnotationTypes(Consts.INTERCEPTOR_ANNOTATION_TYPE)
...
public class InterceptorProcessor extends AbstractProcessor {
    ...
    private boolean validateElement(Element element) {
        return element.getKind().isClass() && processingEnv.getTypeUtils().isAssignable(element.asType(),
                processingEnv.getElementUtils().getTypeElement(Consts.INTERCEPTOR_INTERFACE).asType());
    }
    ...
    private void generateInterceptors(String moduleName, Set<TypeElement> elements) {
         ...
         * params
         */
        TypeElement interceptorType = processingEnv.getElementUtils().getTypeElement(Consts.INTERCEPTOR_INTERFACE);
         ...
         * method
         */
        MethodSpec.Builder handleInterceptors = MethodSpec.methodBuilder(Consts.INTERCEPTORS_METHOD_NAME)
         ...
         * class
         */
        TypeSpec type = TypeSpec.classBuilder(capitalize(moduleName) + Consts.INTERCEPTORS)
        ...
        try {
            JavaFile.builder(Consts.PACKAGE_NAME, type).build().writeTo(processingEnv.getFiler());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
